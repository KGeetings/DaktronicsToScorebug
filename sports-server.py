...
# Third Flask app for stats display hosting
stats_display_app = Flask("stats_display_app", static_url_path="", static_folder=".")
CORS(stats_display_app)
		...
		try:
			...
			if data.get("command") == "clear_all":
				...
				if team in ["home", "guest"]:
					...
					current_game_data[f"{team}_stats_duration"] = data.get(
						"duration", 0
					)
...
class SportDataFetcher:
	def __init__(self, url, sport):
		...
		if self.sport not in SPORT_CONFIGS:
			raise ValueError(
				f"Unsupported sport: {sport}. Supported sports: {list(SPORT_CONFIGS.keys())}"
			)
				...
				if os.path.exists(statsData):
					with open(statsData, "r") as f:
						...
						if "Team1Score" in stats_data:
							current_game_data["home_score"] = str(
								stats_data["Team1Score"]
							)

						if "Team2Score" in stats_data:
							current_game_data["guest_score"] = str(
								stats_data["Team2Score"]
							)
...
@stats_display_app.route("/")
def serve_stats_display():
	return send_file("stats-display.html")

@stats_display_app.route("/basketball_stats.json")
def serve_basketball_stats():
	if os.path.exists("basketball_stats.json"):
		return send_file("basketball_stats.json")
	else:
		return jsonify({"error": "basketball_stats.json not found"}), 404

@stats_display_app.route("/<path:filename>")
def serve_stats_display_static(filename):
	return send_from_directory(".", filename)
...
def run_stats_display_server():
	print("Stats display server running on http://localhost:5003")
	stats_display_app.run(host="0.0.0.0", port=5003)
...
def main():
	...
	print(f"Main server will run on http://localhost:5000")
	print(f"Stats server listening on port 5001")
	print(f"Heatmap server will run on http://localhost:5002")
	print(f"Stats display server will run on http://localhost:5003")
	...
	# Start stats display server in separate thread
	stats_display_thread = threading.Thread(target=run_stats_display_server, daemon=True)
	stats_display_thread.start()
