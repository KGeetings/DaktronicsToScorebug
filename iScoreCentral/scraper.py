...
class iScoreCentralScraper:
	...
	def analyze_scorecast_page(self, iscore_url: str) -> Dict:
		...
		"""
		print(f"Analyzing page: {iscore_url}\n")
	...
	def scrape_from_url(self, iscore_url: str) -> Dict:
		print(f"Attempting to scrape from URL: {iscore_url}")

		try:
			# First, analyze the page to find the correct parameters
			page_info = self.analyze_scorecast_page(iscore_url)

			if "error" in page_info:
				# Fallback: try extracting from URL parameters
				from urllib.parse import urlparse, parse_qs

				parsed_url = urlparse(iscore_url)
				...
				game_guid = page_info.get("game_guid")
				device_guid = page_info.get("device_guid", "")
				customer_guid = page_info.get("customer_guid", "")

			print(f"\nUsing parameters:")
			print(f"  Game GUID: {game_guid}")
			print(f"  Device GUID: {device_guid}")
			print(f"  Customer GUID: {customer_guid}")

			if not game_guid:
				print("Error: Could not extract game GUID")
				return {"error": "Could not extract game GUID"}

			# Now fetch the game data using the extracted GUIDs
			return self.parse_game_data(game_guid, device_guid=device_guid, customer_guid=customer_guid)
...
if __name__ == "__main__":
	...
	print("Scraping game from URL...")
	game_data = scraper.scrape_from_url(game_url)
	...
	if "error" not in game_data and game_data.get("players"):
		...
		for player_guid, player in list(game_data["players"].items())[:5]:
			print(f"  - #{player.player_number} {player.player_name}: {player.stats_today.points} pts")
